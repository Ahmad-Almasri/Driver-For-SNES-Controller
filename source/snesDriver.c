...
{
	/*
	 * prevState: a pointer array that save the previous state of SNESS
	 * data: array to store the current state of the SNESS
	 * index: the index of the pressed btn if index = -1 that means
	 * 	no button was pressed
	 * loop: if loop = 1 that means the while loop is infinite and
	 * 	when loop = 0 that means break the loop 
	 */
	int *prevState	= NULL,
	data[16], index = -1, loop = 1;
	
	GPIO = getGPIOPtr();
	initSNES();
	
	printf("CREATED BY : Ahmad Almasri\n\n");
	...
	while(loop){
		...
		// 	STOP THE GAME
			...
			loop = 0;
		}else if(prevState == NULL && index != -1){
		// 	Firt time : a button pressed
		//	init prevstate Array
			prevState = malloc(16 * sizeof(int));
		//	save the current state
			memcpy(prevState, data, 16*sizeof(int));
		//	print the pressed button
			printf("You have pressed ");
			printPressedBtn(index);
			printf("\n\n");
		//	ask user for an input
			printf("Please press a button ...\n");
		}else if(prevState != NULL && index != -1){
		//	Check if it is the same state or not
			if(compare(data,prevState)==-1){
			// 	Not Equal states
			//	print the pressed button
				printf("You have pressed ");
				printPressedBtn(index);
